...
**熱帯魚水槽の IoT モニタリング × 機械学習プロジェクト**

---

## 🐠 これは何？

自宅の熱帯魚水槽を Raspberry Pi でモニタリングするシステム。
水槽の横に置いた 7インチディスプレイに、水温や照明状態がリアルタイムで表示される。

**ただの監視システムでは終わらせない。**

収集したデータを使って「なぜ水温が変化したのか」を分析し、
最終的には「いつ水換えすべきか」を AI が教えてくれる仕組みを目指している。
...
| 機能 | 状態 |
|------|------|
| 🌡️ **水温のリアルタイム監視** | 水中センサーで 1分ごとに計測 |
| 💡 **照明 ON/OFF の自動記録** | スマートプラグから取得 |
| 🌡️ **室温・湿度の記録** | Tapo センサーから取得 |
| 📊 **Grafana ダッシュボード** | 24時間のトレンドをグラフ表示 |
| 📺 **キオスクモード** | 7インチディスプレイに常時表示 |

![Dashboard](docs/assets/dashboard-preview.png) <!-- TODO: スクリーンショット追加 -->
